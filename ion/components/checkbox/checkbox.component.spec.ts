...
import {
  ...
  CheckBoxProps,
  CheckBoxStates,
} from '@brisanet/ion/components/checkbox';
...
type State = 'enabled' | 'checked' | 'indeterminate';
...
describe('CheckboxComponent', () => {
  ...
  it.each(['enabled', 'checked', 'indeterminate'] as State[])(
    `should render %s disabled`,
    async (state: State) => {
      await sut({ state, disabled: true });
      expect(screen.getByTestId(boxId)).toBeDisabled();
    }
  );
  ...
  it.each(['enabled', 'checked'] as State[])(
...
@NgModule({
  declarations: [CheckboxHostComponent],
  imports: [CommonModule, IonCheckboxComponent],
})
export class CheckboxHostModule {}

describe('Checkbox controlled by a parent component', () => {
  let fixture: ComponentFixture<CheckboxHostComponent>;
  let checkboxHost: CheckboxHostComponent;
  beforeEach(() => {
